...
function PetSelector({ pets, selectedPet, onSelectPet }) {
  ...
  return (
    <div ref={ref} className="relative">
      <Button
        variant="ghost"
        className="justify-between h-auto py-2 px-3 bg-accent/60 hover:bg-accent/80 min-w-[180px]"
        onClick={() => setOpen((v) => !v)}
      >
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-1.5 w-56 rounded-xl border border-border bg-popover shadow-md">
          <div className="p-1">
            {pets.map((pet) => (
              <button
                key={pet.id}
                onClick={() => { onSelectPet(pet.id); setOpen(false); }}
                className={cn(
                  "flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-left text-sm transition-colors hover:bg-accent",
                  pet.id === selectedPet?.id && "bg-accent"
                )}
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-foreground">{pet.breed}</p>
                </div>
              </button>
            ))}
            ...
            <button className="flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-sm transition-colors hover:bg-accent text-foreground">
...
export default function PetDetails() {
  const { pets, vaccinations, allergies, medications, reminders, selectedPetId, setSelectedPetId } = useStore();
      ...
      <div className="space-y-5">
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <button
            ...
            className="inline-flex items-center gap-1.5 w-fit text-sm text-muted-foreground hover:text-foreground transition-colors"
          >
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          ...
          <Card>
            ...
            <CardContent>
              {highSeverityAllergies.length === 0 && petAllergies.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-8 text-muted-foreground">
